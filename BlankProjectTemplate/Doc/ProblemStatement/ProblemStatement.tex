...
Most \wss{Say many instead} of the relationships in nature are linear, that
...
proportioal \wss{remember to spell check.  There are spell checkers for LaTeX} to acceleration and mass is the proportionality constant. For example in Electricity if we take Ohm's Law, \textbf{V = iR} voltage across the conductor is proportional to the current flowing though it and resistance is the proportionality constant. These  examples shows us the importance of linear equations and solving them. In matrix notation the general form of linear equation is

 \centerline{\textbf{Ax = B}}

\wss{$b$ is usually given as lower case.  You'll also want to learn how to write
equations in LaTeX.  :-) }
...
\wss{You should delete the material from the original template file that was
